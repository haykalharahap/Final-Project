        ...
        <div className="md:col-span-1">
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 text-center">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-red-50 text-red-600 py-2 rounded-lg hover:bg-red-100 transition mt-6"
            >
                ...
                {transactions.map((trx) => (
                  <div key={trx.id} className="flex flex-col sm:flex-row justify-between items-center p-4 border rounded-lg hover:bg-gray-50 transition">
                    <div className="mb-2 sm:mb-0 text-center sm:text-left">
                      <p className="font-bold text-gray-800">{trx.invoiceId || trx.id.slice(0, 8)}</p>
                      <p className="text-sm text-gray-500">{new Date(trx.orderDate || trx.createdAt).toLocaleDateString('id-ID')}</p>
                      <p className="text-xs text-gray-400 mt-1">
                        {trx.transaction_items?.map(item => item.food?.name).filter(Boolean).join(", ") || 'No items'}
                      </p>
                    </div>
                    <div className="text-right flex flex-col items-center sm:items-end">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold mb-1 ${trx.status === 'success' ? 'bg-green-100 text-green-700' :
                          trx.status === 'failed' ? 'bg-red-100 text-red-700' :
                            'bg-yellow-100 text-yellow-700'
                        }`}>
                        {trx.status}
                      </span>
                      <span className="font-bold text-orange-600">{formatPrice(trx.totalPrice)}</span>
                    </div>
                  </div>
                ))}
              </div>
