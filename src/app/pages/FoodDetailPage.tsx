...
export function FoodDetailPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
      {/* Back Button */}
      <Link to="/menu" className="inline-flex items-center gap-2 text-gray-600 hover:text-orange-600 mb-4 sm:mb-6 text-sm sm:text-base transition">
      ...
      <div className="flex flex-col md:flex-row gap-6 sm:gap-8 lg:gap-12">
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center">
          ...
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-gray-900 mb-3 sm:mb-4">{food.name}</h1>
          <p className="text-gray-600 text-sm sm:text-base lg:text-lg mb-4 sm:mb-6 leading-relaxed">{food.description}</p>

          <div className="mb-4 sm:mb-6">
            <h3 className="font-bold text-gray-800 mb-2 text-sm sm:text-base">Ingredients:</h3>
            <ul className="flex flex-wrap gap-1.5 sm:gap-2">
              {food.ingredients?.map((ing: string) => (
                <li key={ing} className="bg-gray-100 px-2.5 sm:px-3 py-1 rounded-md text-xs sm:text-sm text-gray-600 border border-gray-200">{ing}</li>
              ))}
            </ul>
          </div>

          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 border-t border-b border-gray-100 py-4 sm:py-6 gap-4 sm:gap-0">
            <div>
              <div className="text-2xl sm:text-3xl font-bold text-orange-600">{formattedPrice}</div>
            ...
            <div className="flex items-center border border-gray-300 rounded-lg">
              <button
                ...
                className="p-2.5 sm:p-3 text-gray-600 hover:text-orange-600 disabled:opacity-50"
                disabled={quantity <= 1}
              >
                <Minus className="w-4 h-4 sm:w-5 sm:h-5" />
              </button>
              <span className="px-3 sm:px-4 font-bold text-base sm:text-lg w-10 sm:w-12 text-center">{quantity}</span>
              <button
                onClick={() => setQuantity(quantity + 1)}
                className="p-2.5 sm:p-3 text-gray-600 hover:text-orange-600"
              >
          ...
          <button
            ...
            className="w-full bg-orange-600 text-white py-3.5 sm:py-4 rounded-xl font-bold text-base sm:text-lg shadow-lg hover:bg-orange-700 transition transform hover:-translate-y-1 flex items-center justify-center gap-2"
          >
      ...
      <div className="mt-10 sm:mt-16">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">Reviews ({ratings.length})</h2>

        {/* Add Review Form */}
        <form onSubmit={handleSubmitReview} className="bg-white p-4 sm:p-6 rounded-xl shadow-md border border-gray-100 mb-6 sm:mb-8">
          <h3 className="font-bold text-gray-800 mb-3 sm:mb-4 text-sm sm:text-base">Write a Review</h3>
          <div className="flex items-center gap-2 mb-3 sm:mb-4">
            <label className="text-xs sm:text-sm font-medium text-gray-700">Rating:</label>
            <div className="flex gap-0.5 sm:gap-1">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className={`p-0.5 sm:p-1 ${star <= ratingValue ? 'text-yellow-500' : 'text-gray-300'}`}
                >
                  <Star className={`w-5 h-5 sm:w-6 sm:h-6 ${star <= ratingValue ? 'fill-current' : ''}`} />
                </button>
              ))}
            </div>
          </div>
          <textarea
            ...
            className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 mb-3 sm:mb-4 text-sm sm:text-base"
            rows={3}
            required
          />
          <button
            ...
            className="bg-orange-600 text-white px-5 sm:px-6 py-2 rounded-lg font-semibold hover:bg-orange-700 transition disabled:opacity-50 text-sm sm:text-base"
          >
        ...
        {ratings.length > 0 ? (
          <div className="space-y-3 sm:space-y-4">
            {ratings.map((r) => (
              <div key={r.id} className="bg-white p-4 sm:p-5 rounded-xl shadow-sm border border-gray-100">
                <div className="flex items-center gap-2 sm:gap-3 mb-2">
                  {r.user?.profilePictureUrl && (
                    <img src={r.user.profilePictureUrl} alt={r.user.name} className="w-7 h-7 sm:w-8 sm:h-8 rounded-full object-cover" />
                  )}
                  <div>
                    <p className="font-semibold text-gray-800 text-sm sm:text-base">{r.user?.name || 'Anonymous'}</p>
                ...
                <p className="text-gray-600 text-sm sm:text-base">{r.review}</p>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-gray-500 text-center py-6 sm:py-8 text-sm sm:text-base">No reviews yet. Be the first to review!</p>
        )}
      </div>
    </div>
  );
